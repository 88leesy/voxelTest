...
#define EPSILON (0.08)  // distance for clustering, metre^2
	...
	int cubeSize = 10;  // Aachen = 15, CMU=ppt 참고, 7Scene = 5, CAU = 5, Cambridge = 10
	...
	*/
	string save_path = "F:/_voxelFeatureMap9/" + dataname + "_3d_nearest_centroid.txt";
	...
	*/
	...
	std::ofstream oStream_first("F:/_voxelFeatureMap9/" + dataname + "db_data_first.txt", ios::out | ios::binary); //ios::app => 이어쓰기
	std::ofstream oStream_second("F:/_voxelFeatureMap9/" + dataname + "db_data_second.txt", ios::out | ios::binary); //ios::app => 이어쓰기
	...
	for (int i = 0; i < ncam - 1; ++i)
	{
		...
		for (int tar_im_id = i + 1; tar_im_id < ncam; ++tar_im_id)
		{
			...
			if (same_pt_cnt >= min_pt_cnt_thresh && same_pt_cnt >= 80)  //특정 개수(최소 70개는 있자) 이상일 때에만 이미지 페어 저장, DB 저장.
			{
				...
				cv::Mat pic16bit;
				ref_label_img.convertTo(pic16bit, CV_16U, 255); //convert to 16-bit by multiplying all values by 255

				cv::Mat pic16bit2;
				tar_label_img.convertTo(pic16bit2, CV_16U, 255); //convert to 16-bit by multiplying all values by 255
				imshow("ref_label_img", pic16bit);
				imshow("tar_label_img", pic16bit2);

				Mat gray; 
				cvtColor(down_img, gray, CV_BGR2GRAY);
				Mat gray2; 
				cvtColor(down_img2, gray2, CV_BGR2GRAY);
				...
				string save_path = "F:/_voxelFeatureMap9/" + dataname + ref_file_name;
				...
				string save_path2 = "F:/_voxelFeatureMap9/" + dataname + tar_file_name;
