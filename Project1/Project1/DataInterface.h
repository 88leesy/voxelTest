...
struct CameraT_
{
    ...
    template <class Float>     void SetQuaternionRotation(const Float q[4])
    {
		...
		//m[0][0] = float_t(1 - 2*qy*qy - 2*qz*qz);
		//m[0][1] = float_t(2 * qx*qy - 2 * qz*qw);
		//m[0][2] = float_t(2 * qy*qw + 2 * qz*qx);
		//m[1][0] = float_t(2 * qx*qy + 2 * qw*qz);
		//m[1][1] = float_t(1 - 2*qx*qx - 2*qz*qz);
		//m[1][2] = float_t(2 * qz*qy - 2 * qx*qw);
		//m[2][0] = float_t(2 * qx*qz - 2 * qy*qw);
		//m[2][1] = float_t(2 * qy*qz + 2 * qw*qx);
		//m[2][2] = float_t(1-2*qx*qx - 2*qy*qy);
    }
    template <class Float>     void GetQuaternionRotation(Float q[4]) const
    {
        q[0]= 1 + m[0][0] + m[1][1] + m[2][2];
		//q[0] = m[0][0] + m[1][1] + m[2][2];
        if(q[0]>0.000000001)
        {
            q[0] = sqrt(q[0])/2.0;
	...
	»óÀ±ÀÇ ÇÔ¼ö
	*/
	template<class Float>	void GetProjectionPoint(Float pt_3d[3], Float pt_2d[2]) const
	{
		float x = m[0][0] * pt_3d[0]
				+ m[0][1] * pt_3d[1]
				+ m[0][2] * pt_3d[2];

		float y = m[1][0] * pt_3d[0]
				+ m[1][1] * pt_3d[1]
				+ m[1][2] * pt_3d[2];

		float z = m[2][0] * pt_3d[0]
				+ m[2][1] * pt_3d[1]
				+ m[2][2] * pt_3d[2];
...
struct Point3D
{
	...
	//template <class Float> Point3D(Float x, Float y, Float z) { SetPoint(x, y, z); }
};
