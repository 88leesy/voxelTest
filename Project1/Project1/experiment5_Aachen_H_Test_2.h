	...
	*/
	...
	for (int i = 0; i < nLabel; ++i)
	{
		for (int j = 0; j < ds.m_points[i].vec_H_voxels.size(); ++j)
		{
			...
			// 벡터 복사
			//최하단 복셀에서, 평면의 개수만큼 나누어진 포인트들을... 한번 더 복셀별로 나눈다... tmp_inlierData.size()=>평면의 개수.., 그리고, 각 평면안에는 DB_Point_3D가 들어가있음
			for (int m = 0; m < tmp_inlierData.size(); ++m)
			{
				...
				//copy(tmp_inlierData[m].begin(), tmp_inlierData[m].end(), std::back_inserter(ds.m_points[i].vec_H_voxels[j].vec_pt_3d_db));
			}
	...
	for (int i = 0; i < nLabel; ++i)
	{
		for (int j = 0; j < ds.m_points[i].vec_H_voxels.size(); ++j)
		{
			...
			{
				vector<Point_3D_DB> &vec_label_points = ds.m_points[i].vec_H_voxels[j].vec_H_voxels[k].vec_pt_3d_db;
		...
		*/
		...
		for (int n = 0; n < nLabel; ++n)
		{
			...
			for (int k = 0; k < ds.m_points[n].vec_H_voxels.size(); ++k)
			{
				...
				{
					//vector<Point_3D_DB> &vec_label_points = ds.m_points[n].vec_H_voxels[k].vec_H_voxels[o].vec_pt_3d_db;
		...
		for (int n = 0; n < nLabel; ++n)
		{
			...
			{
				//j는 각 라벨 번호.
				for (int j = 0; j < eachLabelPt.size(); ++j)  //voxel n에 대한 lv1의 복셀 개수
				{			
					if (n == 22)
						printf("eachLabelPt[j].size():%d\n", eachLabelPt[j].size());
					for (int k = 0; k < eachLabelPt[j].size(); ++k)  //voxel n에 대한 lv1의 복셀 당 나뉘어진 평면의 개수.
					{

						if (n == 22)
							printf("eachLabelPt[%d][%d].size():%d\n", j,k, eachLabelPt[j][k].size());

						if (eachLabelPt[j][k].size() < nThresh_points)  // eachLabelPt[j].size() < 4
							continue;
						...
						int val = eachLabelPt[j][k][0].clusterID;
						...
						for (int yy = 0; yy < label_img.rows; ++yy)
						{
							for (int xx = 0; xx < label_img.cols; ++xx)
							{
								...
								//To do...!
								float nn[3] = { lv1_coefs[n][j][tmp.clusterID - 1][0],
									lv1_coefs[n][j][tmp.clusterID - 1][1], lv1_coefs[n][j][tmp.clusterID - 1][2] };

								float v_dot_n = v[0] * nn[0] + v[1] * nn[1] + v[2] * nn[2];
								if (v_dot_n == 0)
								{
									printf("내적이 0 이므로, 직선과 평면이 나란한 경우다!\n");

									printf("label:%d, v-:%f, %f, %f, ---- n: %f, %f, %f\n", (int)val, v[0], v[1], v[2], nn[0], nn[1], nn[2]);
								}
								float tt = (nn[0] * pt1[0] + nn[1] * pt1[1] + nn[2] * pt1[2] + lv1_coefs[n][j][tmp.clusterID - 1][3])
									/ (nn[0] * (pt1[0] - pt2[0]) + nn[1] * (pt1[1] - pt2[1]) + nn[2] * (pt1[2] - pt2[2]));
				...
				//이 부분 수정 필요. 라벨은 어떻게 줄 것인가. lv1의 복셀에 해당하는 라벨? or 그 하단의 평면 분할에 의한 라벨?? 무엇이 맞을까?
				...
				//여기 아래에 문제가 있겠지? - >To do ,... 2021.08.17
				printf("lv1_coefs.size:%d\n ", lv1_coefs.size());
				printf("lv1_coefs[%d].size:%d\n ", n, lv1_coefs[n].size());
				for (int lb = 0; lb < vec_each_lb_pts.size(); ++lb)
				{
					...
					printf("lb:%d, vec_each_lb_pts[lb].size():%d\n", lb, vec_each_lb_pts[lb].size());
					printf("lv1_coefs[%d][%d].size:%d\n ", n, lb, lv1_coefs[n][lb].size());

					for (int k = 0; k < vec_each_lb_pts[lb].size(); ++k)
					{
						int val = vec_each_lb_pts[lb][k].clusterID;
						printf("k:%d, val:%d\n", k,val);
						...
						//여기 라벨이 안맞음  lv1_coefs[lb][k].size는 0 인데, 할당이 되기 때문에 에러가 생김.
						float nn[3] = { lv1_coefs[n][lb][val - 1][0], lv1_coefs[n][lb][val - 1][1], lv1_coefs[n][lb][val - 1][2] };
						float v_dot_n = v[0] * nn[0] + v[1] * nn[1] + v[2] * nn[2];
						if (v_dot_n == 0)
						{
							printf("내적이 0 이므로, 직선과 평면이 나란한 경우다!\n");
						...
						float tt = (nn[0] * pt1[0] + nn[1] * pt1[1] + nn[2] * pt1[2] + lv1_coefs[n][lb][val - 1][3])
							/ (nn[0] * (pt1[0] - pt2[0]) + nn[1] * (pt1[1] - pt2[1]) + nn[2] * (pt1[2] - pt2[2]));
			...
			for (int j = 0; j < eachLabelPt.size(); ++j)
			{
				for (int k = 0; k < eachLabelPt[j].size(); ++k)
				{
					...
					for (int l = 0; l < eachLabelPt[j].size(); ++l)
					{
						int feature_x = eachLabelPt[j][k][l].x_2d + 0.5;
						int feature_y = eachLabelPt[j][k][l].y_2d + 0.5;
