		...
		*/
		...
		for (int j = 0; j < vec_voxels.size(); ++j)
		{
			...
			for (int k = 0; k < 6; ++k)
			{
				for (int l = 0; l < 4; ++l)
				{
					...
					////GT R|t를 이용한 projection 좌표
					//float x = camera_data[i].m[0][0] * vec_voxels[j].planes[k].x[l]
					//	+ camera_data[i].m[0][1] * vec_voxels[j].planes[k].y[l]
					//	+ camera_data[i].m[0][2] * vec_voxels[j].planes[k].z[l];

					//float y = camera_data[i].m[1][0] * vec_voxels[j].planes[k].x[l]
					//	+ camera_data[i].m[1][1] * vec_voxels[j].planes[k].y[l]
					//	+ camera_data[i].m[1][2] * vec_voxels[j].planes[k].z[l];

					//float z = camera_data[i].m[2][0] * vec_voxels[j].planes[k].x[l]
					//	+ camera_data[i].m[2][1] * vec_voxels[j].planes[k].y[l]
					//	+ camera_data[i].m[2][2] * vec_voxels[j].planes[k].z[l];
		...
		for (int yy = 0; yy < voxel_label_img.rows; ++yy)
		{
			for (int xx = 0; xx < voxel_label_img.cols; ++xx)
			{
				...
				//ushort voxVal = voxel_label_img.at<ushort>(yy,xx);
				//ushort val = label_img.at<ushort>(yy, xx);

				//if (val != 0)
				//{
				//	if (voxVal != val)
				//	{
				//		ref_voxel_label_img.at<ushort>(yy, xx) = val;
				//	}
