...
bool LoadNVM(ifstream& in, string dataname, vector<CameraT>& camera_data, vector<Point3D>& point_data,
	...
	vector<string>& names, vector<int>& ptc, int cubeSize)
{
	//std::ifstream is("db_data_first.bin", ios::in|std::ios::binary);
	//vector<DB_Image> vec_ref_db_image;
	//if (is.is_open())
	//{
	//	while (!is.eof()) // 파일 끝까지 읽었냐...
	//	{
	//		DB_Image db_image;

	//		read_typed_data(is, db_image.img_ID);
	//		int img_path_Size = 0;
	//		read_typed_data(is, img_path_Size);
	//		db_image.img_path.resize(img_path_Size);
	//		is.read(&db_image.img_path[0], img_path_Size);

	//		read_typed_data(is, db_image.quat);
	//		read_typed_data(is, db_image.camCent);
	//		read_typed_data(is, db_image.focal_len);
	//		db_points::readVector(is, db_image.voxel_db_pt);

	//		vec_ref_db_image.push_back(db_image);
	//	}
	//}
	//is.close();

	//std::ifstream is2("db_data_second.bin", ios::in | std::ios::binary);
	//vector<DB_Image> vec_tar_db_image;
	//if (is2.is_open())
	//{
	//	while (!is2.eof()) // 파일 끝까지 읽었냐...
	//	{
	//		DB_Image db_image;

	//		read_typed_data(is2, db_image.img_ID);
	//		int img_path_Size = 0;
	//		read_typed_data(is2, img_path_Size);
	//		db_image.img_path.resize(img_path_Size);
	//		is2.read(&db_image.img_path[0], img_path_Size);

	//		read_typed_data(is2, db_image.quat);
	//		read_typed_data(is2, db_image.camCent);
	//		read_typed_data(is2, db_image.focal_len);
	//		db_points::readVector(is2, db_image.voxel_db_pt);

	//		vec_tar_db_image.push_back(db_image);
	//	}
	//}
	//is2.close();
	...
	//TO do !!! 이거 고쳐야함
	//std::ifstream is("data.txt", ios::in | std::ios::binary);
	//if (is.is_open())
	//{
	//	while (!is.eof()) // 파일 끝까지 읽었냐...
	//	{
	...
	//		while (is.getline(line, sizeof(line))) // 한 줄씩 읽어 처리를 시작한다.
	//		{
	//			cout << line << endl; // 내용 출력
	//		}

	//		int ptSize = 0;
	//		read_typed_data(is, ptSize);
	//		for (int k = 0; k < ptSize; ++k)
	//		{
	//			read_typed_data(is, db_image.voxel_db_pt[k]);
	//		}

	//		int asdf = db_image.img_ID;
	//	}
	//}
	//is.close();
...
#define EPSILON (0.08)  // distance for clustering, metre^2
	...
	*/
	string save_path = "F:/_voxelFeatureMap8/" + dataname + "_3d_nearest_centroid.txt";
	...
	*/
	...
	for (int i = 0; i < ncam; ++i)
	{
		...
		*/
			...
			for (int l = 0; l < vec_label_points[k].size(); ++l)
			{
				...
				for (int m = 0; m < imgs_size; ++m)
				{
					if (vec_label_points[k][l].img_IDs[m] == i)
					{
						...
						feature_x = int(feature_x / (1920.0 / reSize_W));
						feature_y = int(feature_y / (1080.0 / reSize_H));
		...
		//	feature_x = feature_x / (1920.0 / reSize_W);
		//	feature_y = feature_y / (1080.0 / reSize_H);

		//	kp_label_img2.data[feature_y*kp_label_img2.cols + feature_x] = 255;
		//}
		//imshow("eachlabelpt", kp_label_img);
		//imshow("ori keypt", kp_label_img2);
		//waitkey();
		...
		double scale = 1080.0 / reSize_H;
		focal_len = focal_len / scale;  //oldhospital=1660, shopfacade=1672
		tmp_db_img.focal_len = focal_len;
		...
		for (int j = 0; j < eachLabelPt.size(); ++j)
		{
			...
			{
				tmp_db_img.voxel_db_pt.push_back(eachLabelPt[j][k]);
			}
		}
	...
	std::ofstream oStream_first("F:/_voxelFeatureMap8/" + dataname + "db_data_first.txt", ios::out | ios::binary); //ios::app => 이어쓰기
	std::ofstream oStream_second("F:/_voxelFeatureMap8/" + dataname + "db_data_second.txt", ios::out | ios::binary); //ios::app => 이어쓰기
	...
	for (int i = 0; i < ncam-1; ++i)
	{
		...
		int ref_vox_db_size = vec_db_image[i].voxel_db_pt.size();

		int pair_cnt = 0;
		
		//for (int tar_im_id = 0; tar_im_id < ncam; ++tar_im_id)
		for (int tar_im_id = i + 1; tar_im_id < ncam; ++tar_im_id)
		{
			if (pair_cnt > 10)  // pair쌍 너무 많으면 다음 레퍼런스... 너무 많으니깐 ^^
				break;

			string img_path2 = "F:/RPNet_test/data/absolute_cambridge/" + dataname + names[tar_im_id];
			//cout << "tar: " << names[tar_im_id] << endl;

			int nExt2 = img_path2.rfind("jpg");
			int nName2 = img_path2.rfind("/") + 1;
			string strReName2;
			strReName2 = img_path2.substr(0, nName2);
			strReName2 += img_path2.substr(nName2, nExt2 - nName2);
			strReName2 += strModExt;

			Mat img2 = imread(strReName2);
			...
			int tar_vox_db_size = vec_db_image[tar_im_id].voxel_db_pt.size();
			int same_pt_cnt = 0;
			int corresp_threshold = MIN(ref_vox_db_size, tar_vox_db_size) * 0.5 ;
			
			DB_Image ref_tmp_dbImg, tar_tmp_dbImg;
			vector<DB_Point> ref_tmp_dbPt, tar_tmp_dbPt;
			...
			for (int idx = 0; idx < ref_vox_db_size; ++idx)
			{
				//int ref_pt_2d_id = vec_db_image[i].voxel_db_pt[idx].pt_2d_id;   //pt_2d_id는 무쓸모?? 이거 다시 보자... 일단 3d_id로 진행!
				int ref_pt_3d_id = vec_db_image[i].voxel_db_pt[idx].pt_3d_id;  

				for (int idx2 = 0; idx2 < tar_vox_db_size; ++idx2)
				{	
					//int tar_pt_2d_id = vec_db_image[tar_im_id].voxel_db_pt[idx2].pt_2d_id; //pt_2d_id는 무쓸모?? 이거 다시 보자... 일단 3d_id로 진행!
					int tar_pt_3d_id = vec_db_image[tar_im_id].voxel_db_pt[idx2].pt_3d_id;

					if (ref_pt_3d_id == tar_pt_3d_id)        //같은게 나오면 그 다음은 없기 때문에,  target의 루프를 탈출 한다.
					{
						++same_pt_cnt;
						
						//float feature_x2 = vec_db_image[tar_im_id].voxel_db_pt[idx2].x_2d;
						//float feature_y2 = vec_db_image[tar_im_id].voxel_db_pt[idx2].y_2d;
						////Scalar cvColor2(255, 0, 0);
						////circle(down_img2, Point(feature_x2, feature_y2), 2, cvColor2, -1);

						//float feature_x = vec_db_image[i].voxel_db_pt[idx].x_2d;
						//float feature_y = vec_db_image[i].voxel_db_pt[idx].y_2d;
						////Scalar cvColor(0, 0, 255);
						////circle(down_img, Point(feature_x, feature_y), 2, cvColor, -1);

						//kp_label_img.data[(int)feature_y*kp_label_img.cols + (int)feature_x] = 255;
						//kp_label_img2.data[(int)feature_y2*kp_label_img2.cols + (int)feature_x2] = 255;

						////printf("idx:%d, idx2:%d, ref_vox_db_size:%d, tar_vox_db_size:%d\n", idx, idx2, ref_vox_db_size, tar_vox_db_size);

						//ref_tmp_dbPt.push_back(vec_db_image[i].voxel_db_pt[idx]);
						//tar_tmp_dbPt.push_back(vec_db_image[tar_im_id].voxel_db_pt[idx2]);

						int ref_lb = vec_db_image[i].voxel_db_pt[idx].clusterID-1;
						ref_label_tmp_dbPt[ref_lb].push_back(vec_db_image[i].voxel_db_pt[idx]);

						int tar_lb = vec_db_image[tar_im_id].voxel_db_pt[idx2].clusterID - 1;
						tar_label_tmp_dbPt[tar_lb].push_back(vec_db_image[tar_im_id].voxel_db_pt[idx2]);
			...
			//if (same_pt_cnt < 30)  // 대응점의 개수가 너무 적다. 다음 레퍼런스 영상을 얻자.(사실 다 검색 해봐야하지만.. 시간이 너무 오래 걸림 ㅠㅠ)
			//{
			//	//printf("대응점 개수가 50개도 안됩니다.\n");
			...
			if (same_pt_cnt > corresp_threshold && same_pt_cnt>=50)  //특정 개수(최소 50개는 있자)와 비율로.. 이상일 때에만 이미지 페어 저장, DB 저장.
			{
				...
				{
					if (ref_label_tmp_dbPt[j].size() < 5)   //이 부분이 voxel labeling convexhull 잡아주는 개수 부분... 1말고... 딴걸루 하장...
						continue;
					...
					for (int k = 0; k < ref_label_tmp_dbPt[j].size(); ++k)
					{
						...
						kp_label_img.data[(int)feature_y*kp_label_img.cols + (int)feature_x] = 255;
						ref_tmp_dbPt.push_back(ref_label_tmp_dbPt[j][k]);
				...
				{
					if (tar_label_tmp_dbPt[j].size() < 5) //이 부분이 target voxel labeling convexhull 잡아주는 개수 부분... 1말고... 딴걸루 하장...
						continue;
					...
					for (int k = 0; k < tar_label_tmp_dbPt[j].size(); ++k)
					{
						...
						tar_tmp_dbPt.push_back(tar_label_tmp_dbPt[j][k]);

						//circle(down_img2, Point(feature_x, feature_y), 1, cvColor, -1);
					...
					const Point *pts = (const Point*)Mat(hull).data;
					int npts = Mat(hull).rows;
					//polylines(down_img2, &pts, &npts, 1, isClosed, cvColor, 1);
				...
				//imshow("ref label", ref_label_img);
				//imshow("tar label", tar_label_img);
				//waitKey();
				...
				string ref_file_name = line_str[0] + "/" + to_string(i) + '_' + to_string(tar_im_id) + '_' + "ref_" + line_str[0] + '_' + line_str[1];
				ref_file_name.replace(ref_file_name.find("jpg"), 3, "png");

				line_str = split(names[tar_im_id], '/');
				string tar_file_name = line_str[0] + "/" + to_string(i) + '_' + to_string(tar_im_id) + '_' + "tar_" + line_str[0] + '_' + line_str[1];
				tar_file_name.replace(tar_file_name.find("jpg"), 3, "png");
				

				string save_path = "F:/_voxelFeatureMap8/" + dataname + ref_file_name;
				imwrite(save_path, down_img);
				save_path.replace(save_path.find("frame"), 5, "label_frame");
				imwrite(save_path, ref_label_img);
				save_path.replace(save_path.find("label"), 5, "kp");		
				imwrite(save_path, kp_label_img);

				string save_path2 = "F:/_voxelFeatureMap8/" + dataname + tar_file_name;
				imwrite(save_path2, down_img2);
				save_path2.replace(save_path2.find("frame"), 5, "label_frame");
				imwrite(save_path2, tar_label_img);
				save_path2.replace(save_path2.find("label"), 5, "kp");	
				imwrite(save_path2, kp_label_img2);


				//imshow("ref ", down_img);
				//imshow("tar ", down_img2);
				//imshow(ref_file_name, kp_label_img);
				//imshow(tar_file_name, kp_label_img2);
				//waitKey();

				pair<DB_Image, DB_Image> pair_DB_img;
				...
				oStream_first << pair_DB_img.first.img_ID << " " << pair_DB_img.first.img_path << " " << pair_DB_img.first.quat[0] << " " << pair_DB_img.first.quat[1]
					...
					<< " " << pair_DB_img.first.camCent[2] << " " << pair_DB_img.first.focal_len << " " << pair_DB_img.first.voxel_db_pt.size() << endl;

				for (int k = 0; k < pair_DB_img.first.voxel_db_pt.size(); ++k)
				{
					oStream_first << pair_DB_img.first.voxel_db_pt[k].x_2d << " " << pair_DB_img.first.voxel_db_pt[k].y_2d
						<< " " << pair_DB_img.first.voxel_db_pt[k].x << " " << pair_DB_img.first.voxel_db_pt[k].y << " " << pair_DB_img.first.voxel_db_pt[k].z
						<< " " << pair_DB_img.first.voxel_db_pt[k].clusterID << " " << pair_DB_img.first.voxel_db_pt[k].pt_3d_id << endl;
				}
				...
				oStream_second << pair_DB_img.second.img_ID << " " << pair_DB_img.second.img_path << " " << pair_DB_img.second.quat[0] << " " << pair_DB_img.second.quat[1]
					...
					<< " " << pair_DB_img.second.camCent[2] << " " << pair_DB_img.second.focal_len << " " << pair_DB_img.second.voxel_db_pt.size() << endl;

				for (int k = 0; k < pair_DB_img.second.voxel_db_pt.size(); ++k)
				{
					oStream_second << pair_DB_img.second.voxel_db_pt[k].x_2d << " " << pair_DB_img.second.voxel_db_pt[k].y_2d
						<< " " << pair_DB_img.second.voxel_db_pt[k].x << " " << pair_DB_img.second.voxel_db_pt[k].y << " " << pair_DB_img.second.voxel_db_pt[k].z
						<< " " << pair_DB_img.second.voxel_db_pt[k].clusterID << " " << pair_DB_img.second.voxel_db_pt[k].pt_3d_id << endl;
				}


				//bin파일로 저장하는 방법
				//write_typed_data(oStream_first, pair_DB_img.first.img_ID);
				//int img_path_Size = pair_DB_img.first.img_path.size();
				//write_typed_data(oStream_first, img_path_Size);
				//oStream_first.write(pair_DB_img.first.img_path.c_str(), pair_DB_img.first.img_path.size());
				//write_typed_data(oStream_first, pair_DB_img.first.quat);
				//write_typed_data(oStream_first, pair_DB_img.first.camCent);
				//write_typed_data(oStream_first, pair_DB_img.first.focal_len);
				//db_points::writeVector(oStream_first, pair_DB_img.first.voxel_db_pt);

				//write_typed_data(oStream_second, pair_DB_img.second.img_ID);
				//img_path_Size = pair_DB_img.second.img_path.size();
				//write_typed_data(oStream_second, img_path_Size);
				//oStream_second.write(pair_DB_img.second.img_path.c_str(), pair_DB_img.second.img_path.size());
				//write_typed_data(oStream_second, pair_DB_img.second.quat);
				//write_typed_data(oStream_second, pair_DB_img.second.camCent);
				//write_typed_data(oStream_second, pair_DB_img.second.focal_len);
				//db_points::writeVector(oStream_second, pair_DB_img.second.voxel_db_pt);
