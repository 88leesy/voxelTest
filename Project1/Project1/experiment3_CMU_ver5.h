...
#define EPSILON (0.08)  // distance for clustering, metre^2
	...
	float reSize_H = 768/4, reSize_W = 1024/4;  //원본 1024x768, 256x192
	...
	*/
	...
	for (int i = 0; i < nLabel; ++i)
	{
		...
		float coef[4] = { 0 };
		//float meanErr = ds.PlaneFitting_multi(vec_label_points[i], vec_label_points[i].size(), tmp_inlierData, color, coef);
	...
	*/
	string save_3dpt_path = "F:/_voxelFeatureMap_CMU2/_single_Voxel_set/" + dataname + "_3d_nearest_centroid.txt";
	...
	std::ofstream oStream_dbData("F:/_voxelFeatureMap_CMU2/_single_Voxel_set/" + dataname + "db_data.txt", ios::out | ios::binary); //ios::app => 이어쓰기
	...
	for (int i = 0; i < ncam; ++i)
	{
		...
		*/
			...
			for (int l = 0; l < vec_label_points[k].size(); ++l)
			{
				...
				for (int m = 0; m < imgs_size; ++m)
				{
					if (vec_label_points[k][l].img_IDs[m] == i)
					{
						...
						//printf("x:%f, y:%f, - x:%f, y:%f, -- X:%f, Y:%f, Z:%f\n",
						//	feature_x, feature_y, vec_label_points[k][l].vec_2d_pt[m].x, vec_label_points[k][l].vec_2d_pt[m].y, 
						//	tmp_db.x, tmp_db.y, tmp_db.z);
					}
			...
			for (int j = 0; j < eachLabelPt.size(); ++j)
			{
				...
				for (int yy = 0; yy < label_img.rows; ++yy)
				{
					for (int xx = 0; xx < label_img.cols; ++xx)
					{
						...
						Point2f_vec.at<double>(0) = (feature_x); //- donw_c_w) ; // DB_img.focal_lenX;	 / DB_img.focal_lenY											
		...
		//	feature_x = feature_x / (img.cols / down_img.cols);
		//	feature_y = feature_y / (img.rows / down_img.rows);
		...
		for (int yy = 0; yy < label_img.rows; ++yy)
		{
			for (int xx = 0; xx < label_img.cols; ++xx)
			{
				...
				cv::Mat Point2f_vec = cv::Mat(3, 1, CV_64FC1);
				Point2f_vec.at<double>(0) = (xx); //- donw_c_w); // DB_img.focal_lenX, 		 / DB_img.focal_lenY;									
				Point2f_vec.at<double>(1) = (yy); //- donw_c_h);
				Point2f_vec.at<double>(2) = 1;
				cv::Mat point_3d = AP_mat_inv * Point2f_vec;
				point_3d.at<double>(0) = point_3d.at<double>(0) / point_3d.at<double>(3);
				point_3d.at<double>(1) = point_3d.at<double>(1) / point_3d.at<double>(3);
				...
				int xxx = Point2f.x; //+donw_c_w;
				int yyy = Point2f.y; //+donw_c_h;
				...
				//vec_features_2d.push_back(Point2d(xxx - donw_c_w, yyy - donw_c_h));
		...
		//vec_features_2d.clear();
		//vec_features_3d.clear();
		...
		//		printf("x:%f, y:%f, -- X:%f, Y:%f, Z:%f\n",  feature_x, feature_y, x, y, z);
		//	}
		//}

		//printf("focal_lenX:%f, focal_lenY:%f, -- Cx:%f, Cy:%f\n", DB_img.focal_lenX, DB_img.focal_lenY, DB_img.Cx, DB_img.Cy);
		bool correspondence = EstimatePoseByPnP(vec_features_2d, vec_features_3d,
			DB_img.focal_lenX, DB_img.focal_lenY, DB_img.Cx, DB_img.Cy, A_matrix, R_matrix, t_matrix);
		...
		string save_path_rgb = "F:/_voxelFeatureMap_CMU2/_single_Voxel_set/" + dataname + names[i];
		string save_path = "F:/_voxelFeatureMap_CMU2/_single_Voxel_set/" + dataname + line_str[0] + "/label_" + line_str[1];
		string save_path2 = "F:/_voxelFeatureMap_CMU2/_single_Voxel_set/" + dataname + line_str[0] + "/kp_" + line_str[1];
		string save_path3 = "F:/_voxelFeatureMap_CMU2/_single_Voxel_set/" + dataname + line_str[0] + "/3dpts_" + line_str[1]; 
		string save_path4 = "F:/_voxelFeatureMap_CMU2/_single_Voxel_set/" + dataname + line_str[0] + "/contour_" + line_str[1]; 
		string save_path5 = "F:/_voxelFeatureMap_CMU2/_single_Voxel_set/" + dataname + line_str[0] + "/fillcontour_" + line_str[1]; 
		save_path_rgb.replace(save_path_rgb.find("jpg"), 3, "png");
		save_path.replace(save_path.find("jpg"), 3, "png");
		save_path2.replace(save_path2.find("jpg"), 3, "png");
		save_path3.replace(save_path3.find("jpg"), 3, "yml");
		save_path4.replace(save_path4.find("jpg"), 3, "png");
		save_path5.replace(save_path5.find("jpg"), 3, "png");
		...
		oStream_dbData << DB_img.img_ID << " " << DB_img.img_path << " " << DB_img.quat[0] << " " << DB_img.quat[1]
			...
			<< " " << DB_img.camCent[2] << " " << DB_img.focal_lenX << " " << DB_img.focal_lenY << " " << DB_img.Cx << " " << DB_img.Cy << " ";
