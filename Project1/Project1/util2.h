...
#define EPSILON (0.08)  // distance for clustering, metre^2
	...
	ds.voxelFitting2(ds.m_points, ds.getTotalPointSize(), color, nLabel);
	...
	label별로 묶기
	*/
	vector<DB_Point> vec_centroid_points;
	...
	for (int i = 0; i < num_points; ++i)
	{
		int label = tmp_points[i].clusterID;
	...
	for (int i = 0; i < nLabel; ++i)
	{
		...
		centroidPt = vec_label_points[i][centId];
		vec_centroid_points.push_back(centroidPt);

		printf("centId:%d, minDist:%f, vec_label_points[i][j]:%d\n", centId, minDist, vec_label_points[i].size());
	}
	...
	for (int i = 0; i < ncam; ++i)
	{
		...
		*/
		for (int j = 0; j < size; ++j)
		{
			...
			{
				if (features_3d[i][j].x == tmp_points[k].x
					&& features_3d[i][j].y == tmp_points[k].y
					&& features_3d[i][j].z == tmp_points[k].z)
				{
					int feature_x = features_2d[i][j].x + c_w + 0.5;
					int feature_y = features_2d[i][j].y + c_h + 0.5; 

					int color_lb = tmp_points[k].clusterID;
					int r = color[color_lb][0];
					int g = color[color_lb][1];
					int b = color[color_lb][2];

					//circle(img, Point(feature_x, feature_y), 4, Scalar(b, g, r), -1);
					//char txt[20];    // 크기가 20인 char형 배열을 선언
					//sprintf(txt, "%d", color_lb);
					//putText(img, txt, Point(feature_x, feature_y), CV_FONT_HERSHEY_SIMPLEX, 0.5, Scalar(b, g, r));
					

					//
					float pt[3];
					//pt[0] = vec_centroid_points[color_lb].x;
					//pt[1] = vec_centroid_points[color_lb].y;
					//pt[2] = vec_centroid_points[color_lb].z;
					...
					//circle(img, Point(_xx, _yy), 4, Scalar(b, g, r), -1);
					//char txt[20];    // 크기가 20인 char형 배열을 선언
					//sprintf(txt, "%d", color_lb);
					//putText(img, txt, Point(_xx, _yy), CV_FONT_HERSHEY_SIMPLEX, 0.5, Scalar(b, g, r));

					//printf("--------- vec_label_points[color_lb].size(): %d -----------\n ", vec_label_points[color_lb].size());
					for (int l = 0; l < vec_label_points[color_lb].size(); ++l)
					{
						pt[0] = vec_label_points[color_lb][l].x;
						pt[1] = vec_label_points[color_lb][l].y;
						pt[2] = vec_label_points[color_lb][l].z;
						float pt_2d[2] = { 0 };
						camera_data[i].GetProjectionPoint(pt, pt_2d);
						float xx = pt_2d[0];
						float yy = pt_2d[1];
