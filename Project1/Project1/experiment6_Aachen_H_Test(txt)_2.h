...
#define EPSILON (0.08)  // distance for clustering, metre^2
	...
	ds.H_voxelFitting_allCnt(ds.m_points, vec_voxels, ds.getTotalPointSize(), color, nLabel); 


	int num_points = 0;
	int lv1_lbSize = ds.m_points.size();
	int lv2_lbMaxSize = 0;
	for (int n = 0; n < ds.m_points.size(); ++n)
	{
		if (lv2_lbMaxSize < ds.m_points[n].vec_H_voxels.size())
			lv2_lbMaxSize = ds.m_points[n].vec_H_voxels.size();
		for (int p = 0; p < ds.m_points[n].vec_H_voxels.size(); ++p)
		{
			num_points += ds.m_points[n].vec_H_voxels[p].vec_pt_3d_db.size();
		}
	}
	printf("\n refined num_points:%d, lv1_lbSize:%d, lv2_lbMaxSize:%d\n", num_points, lv1_lbSize, lv2_lbMaxSize);
	...
	for (int i = 0; i < nLabel; ++i)
	{
		for (int j = 0; j < ds.m_points[i].vec_H_voxels.size(); ++j)
		{
			...
			vector<DB_Point_3D> &vec_label_points = ds.m_points[i].vec_H_voxels[j].vec_pt_3d_db;

			ds.m_points[i].vec_H_voxels[j].coef[0] = 0;
			ds.m_points[i].vec_H_voxels[j].coef[1] = 0;
			ds.m_points[i].vec_H_voxels[j].coef[2] = 0;
			ds.m_points[i].vec_H_voxels[j].coef[3] = 0;
	...
	//graph-txt로 저장하기...
	string vox_save_path = absPath + "voxel_graph.txt";
	...
	*/
	...
	for (int i = 0; i < ncam; ++i)
	{
		...
		Mat label_img_lv0 = Mat::zeros(down_img.rows, down_img.cols, CV_16U);
		...
		*/
		...
		//여기 레벨별로 라벨링 할 것이기 때문에... 코딩 다시하장...

		double time1 = cv::getTickCount();

		vector<vector<DB_POINT>> eachLabelPt_lv0;
		eachLabelPt_lv0.resize(nLabel);
		for (int n = 0; n < nLabel; ++n)
		{
			//printf("n:%d\n", n);

			vector<DB_Point_3D> vec_label_points_lv0 = ds.m_points[n].vec_pt_3d_db; //각 평면안에 들어오는 포이트 수.
			// level 0단계...
			if (vec_label_points_lv0.size() < nThresh_points)  // 복셀안의 포인트 수가 4개 미만이면, 진행 X
			{
				continue;
			}

			for (int l = 0; l < vec_label_points_lv0.size(); ++l)
			{
				int imgs_size = vec_label_points_lv0[l].img_IDs.size();

				for (int m = 0; m < imgs_size; ++m)
				{
					//printf("vec_label_points_lv0[%d].img_IDs[%d]:%d\n", m, vec_label_points_lv0[l].img_IDs[m]);

					if (vec_label_points_lv0[l].img_IDs[m] == vec_img_ids[i])
					{
						float feature_x = vec_label_points_lv0[l].vec_2d_pt[m].x;
						float feature_y = vec_label_points_lv0[l].vec_2d_pt[m].y;
						...
						DB_POINT tmp_db;
						int color_lb = vec_label_points_lv0[l].clusterID - 1;
						tmp_db.clusterID = color_lb + 1;  //배경을 0 라벨로 주기위해... 
						tmp_db.x = vec_label_points_lv0[l].x;
						tmp_db.y = vec_label_points_lv0[l].y;
						tmp_db.z = vec_label_points_lv0[l].z;
						tmp_db.x_2d = feature_x;
						tmp_db.y_2d = feature_y;
						tmp_db.pt_3d_id = vec_label_points_lv0[l].pt_3d_id;
						tmp_db.pt_2d_id = vec_label_points_lv0[l].pt_2d_ids[m];
						eachLabelPt_lv0[n].push_back(tmp_db);
					}
				}
			}
			...
			vector<vector<DB_POINT>> eachLabelPt_lv1;
			int ss = ds.m_points[n].vec_H_voxels.size();
			eachLabelPt_lv1.resize(ss);   //level0의 각 복셀의 아래단계인 level1의 복셀 개수(라벨)

			//계층적 복셀에...전부다 each로 만들어서...? 흠.... 생각해라.
			for (int k = 0; k < ds.m_points[n].vec_H_voxels.size(); ++k)
			{
				vector<DB_Point_3D> &vec_label_points = ds.m_points[n].vec_H_voxels[k].vec_pt_3d_db; //각 평면안에 들어오는 포이트 수.
				//float coef[4];
				//coef[0] = ds.m_points[n].vec_H_voxels[k].coef[0];
				//coef[1] = ds.m_points[n].vec_H_voxels[k].coef[1];
				//coef[2] = ds.m_points[n].vec_H_voxels[k].coef[2];
				//coef[3] = ds.m_points[n].vec_H_voxels[k].coef[3];
				...
				for (int l = 0; l < vec_label_points.size(); ++l)
				{
					...
					for (int m = 0; m < imgs_size; ++m)
					{
						//printf("vec_label_points[%d].img_IDs[%d]:%d\n", m, vec_label_points[l].img_IDs[m]);
						...
						if (vec_label_points[l].img_IDs[m] == vec_img_ids[i])
						{
							...
							//printf("k:%d, color_lb :%d, lb2:%d\n", k, color_lb, ds.m_points[n].vec_H_voxels[k].nLabel);

							eachLabelPt_lv1[k].push_back(tmp_db);

							//printf("clusterID:%d, coef[0]:%f, lb2:%d\n", tmp_db.clusterID, tmp_db.coef[0], tmp_db.pt_3d_id);
			...
			for (int j = 0; j < eachLabelPt_lv1.size(); ++j)
			{
				if (eachLabelPt_lv1[j].size() < nThresh_points)
					continue;

				DB_img.voxel_db_pt.push_back(eachLabelPt_lv1[j]);

				vector<Point> contour, resized_contour;
				Scalar cvColor(color[j][0], color[j][1], color[j][2]);
				//printf("eachLabelPt_lv1[%d].size() : %d\n", j, eachLabelPt_lv1[j].size());
				for (int l = 0; l < eachLabelPt_lv1[j].size(); ++l)
				{
					int feature_x = eachLabelPt_lv1[j][l].x_2d + 0.5;
					int feature_y = eachLabelPt_lv1[j][l].y_2d + 0.5;
					...
					vec_features_3d.push_back(Point3d(eachLabelPt_lv1[j][l].x, eachLabelPt_lv1[j][l].y, eachLabelPt_lv1[j][l].z));
				...
				ushort val = eachLabelPt_lv1[j][0].clusterID;
		...
		//// lv0에 대한 라벨링
		//for (int j = 0; j < eachLabelPt_lv0.size(); ++j)
		//{
		//	if (eachLabelPt_lv0[j].size() < nThresh_points)
		//		continue;

		//	//DB_img.voxel_db_pt.push_back(eachLabelPt_lv0[j]);

		//	vector<Point> contour, resized_contour;
		//	Scalar cvColor(color[j][0], color[j][1], color[j][2]);
		//	for (int l = 0; l < eachLabelPt_lv0[j].size(); ++l)
		//	{
		//		int feature_x = eachLabelPt_lv0[j][l].x_2d + 0.5;
		//		int feature_y = eachLabelPt_lv0[j][l].y_2d + 0.5;

		//		circle(down_img_lv0, Point(feature_x, feature_y), 1, cvColor, -1);
		//		resized_contour.push_back(Point(feature_x, feature_y));
		...
		//	const Point *pts = (const Point*)Mat(hull).data;
		//	int npts = Mat(hull).rows;
		//	polylines(down_img_lv0, &pts, &npts, 1, isClosed, cvColor, 1);

		//	ushort val = eachLabelPt_lv0[j][0].clusterID;
		//	fillPoly(label_img_lv0, &pts, &npts, 1, Scalar(val));
		//}
		...
		if (0)
		{
			...
			imshow("kp_label_img", kp_label_img);
			imshow("down_img", down_img);
			imshow("down_img-lv0", down_img_lv0);
			cv::Mat pic16bit;
			label_img.convertTo(pic16bit, CV_16U, 255); //convert to 16-bit by multiplying all values by 255
			cv::namedWindow("label_img");
			cv::imshow("label_img", pic16bit);
