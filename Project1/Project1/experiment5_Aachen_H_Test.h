	...
	cv::Mat &A_matrix, cv::Mat &R_matrix, cv::Mat &t_matrix)
{
	...
	bool correspondence = cv::solvePnPRansac(list_points3d, list_points2d, A_matrix, distCoeffs, rvec, tvec, false, 
		5000, 12, 0.99, inliers, SOLVEPNP_P3P);  //SOLVEPNP_EPNP, SOLVEPNP_P3P
...
#define NUM_COLOR 10000
	...
	if (0)
	{
		...
		for (int idx = 0; idx < ncam; ++idx)
		{
			...
			//reprojection points
			double k1 = camera_data[idx].radial;
			double k2 = 0;
			double p1 = 0;
			double p2 = 0;
			double d[] = { k1, k2, p1, p2 };
			...
			cv::Mat tvec = cv::Mat::zeros(3, 1, CV_64FC1);
			bool useExtrinsicGuess = true;
			correspondence = cv::solvePnPRansac(features_3d[idx], features_2d[idx], A_matrix, distCoeffs, rvec, tvec, false,
				5000, 12, 0.99, inliers, SOLVEPNP_P3P);  //SOLVEPNP_EPNP, SOLVEPNP_P3P
			...
			if (correspondence)
			{
				...
				for (int i = 0; i < num_inliers; ++i)
				{
					refi_list_points2d.emplace_back(features_2d[idx][inliers.at<int>(i)]);
					refi_list_points3d.emplace_back(features_3d[idx][inliers.at<int>(i)]);
				}

				correspondence = cv::solvePnP(refi_list_points3d, refi_list_points2d, A_matrix, distCoeffs, rvec, tvec,
					useExtrinsicGuess, SOLVEPNP_ITERATIVE);
			}
			std::vector<cv::Point2d> projections;
			cv::projectPoints(features_3d[idx], rvec, tvec, A_matrix, distCoeffs, projections);
				...
				//for (int ind = 0; ind < tmp_features_3d.size(); ++ind)
			{
				...
				circle(img, Point(xxx, yyy), 2, Scalar(255, 0, 0), -1);
				circle(img, projections.at(ind), 2, Scalar(0, 255, 0), -1);
				...
				//circle(img, Point(xx, yy), 2, Scalar(0, 255, 0), -1);
				...
				circle(img, Point(feature_x, feature_y), 2, Scalar(0, 0, 255), -1);

				//float GT_error = sqrt((xxx - feature_x)*(xxx - feature_x) + (yyy - feature_y)*(yyy - feature_y));
				float GT_error = sqrt((projections.at(ind).x - feature_x)*(projections.at(ind).x - feature_x)
					+ (projections.at(ind).y - feature_y)*(projections.at(ind).y - feature_y));
...
#define EPSILON (0.08)  // distance for clustering, metre^2
	...
	*/
	...
	//	float coef[4] = { 0 };
	//	//float meanErr = ds.PlaneFitting_Hierarchy(ds.m_points[i].vec_pt_3d_db, ds.m_points[i].vec_pt_3d_db.size(),
	//	//	tmp_inlierData, 0.3, color, coef);
	//	float meanErr = ds.PlaneFitting_Hierarchy_ver2(ds.m_points[i].vec_pt_3d_db, ds.m_points[i].vec_pt_3d_db.size(),
	//		tmp_inlierData, 0.3, color, coef);
	//	sumFit_distErr += meanErr;

	//	int lb = ds.m_points[i].vec_pt_3d_db[0].clusterID;

	//	coefs[i][0] = coef[0]; coefs[i][1] = coef[1]; coefs[i][2] = coef[2]; coefs[i][3] = coef[3];
	//	ds.m_points[i].coef[0] = coef[0]; ds.m_points[i].coef[1] = coef[1]; 
	//	ds.m_points[i].coef[2] = coef[2]; ds.m_points[i].coef[3] = coef[3];
	//	printf("i:%d- %f, %f, %f, %f - lb:%d\n", i, coefs[i][0], coefs[i][1], coefs[i][2], coefs[i][3], lb);

	//	printf("before ds.m_points[i].vec_pt_3d_db size : %d\n", ds.m_points[i].vec_pt_3d_db.size());

	//	// 벡터 복사
	//	ds.m_points[i].vec_pt_3d_db.clear();
	//	ds.m_points[i].vec_pt_3d_db.resize(tmp_inlierData.size());
	//	copy(tmp_inlierData.begin(), tmp_inlierData.end(), ds.m_points[i].vec_pt_3d_db.begin());

	//	printf("after ds.m_points[i].vec_pt_3d_db size : %d\n\n", ds.m_points[i].vec_pt_3d_db.size());
	//}
	//printf("level0 -avgFit_distErr:%f\n", sumFit_distErr / nLabel);
	//printf("vec_inlierData:%d\n", ds.m_points.size());
	////ds.printResults(ds.m_points, ds.m_points.size(), true, color);  //Display
	...
	for (int i = 0; i < nLabel; ++i)
	{
		for (int j = 0; j < ds.m_points[i].vec_H_voxels.size(); ++j)
		{
			vector<DB_Point_3D> tmp_inlierData;
			float coef[4] = { 0 };
			float meanErr = ds.PlaneFitting_Hierarchy(ds.m_points[i].vec_H_voxels[j].vec_pt_3d_db, 
				ds.m_points[i].vec_H_voxels[j].vec_pt_3d_db.size(), tmp_inlierData, 0.1, color, coef);

			int lb = ds.m_points[i].vec_H_voxels[j].nLabel;
			int lb2 = ds.m_points[i].vec_H_voxels[j].vec_pt_3d_db[0].clusterID;

			//coefs[i][0] = coef[0]; coefs[i][1] = coef[1]; coefs[i][2] = coef[2]; coefs[i][3] = coef[3];
			ds.m_points[i].vec_H_voxels[j].coef[0] = coef[0]; ds.m_points[i].vec_H_voxels[j].coef[1] = coef[1];
			ds.m_points[i].vec_H_voxels[j].coef[2] = coef[2]; ds.m_points[i].vec_H_voxels[j].coef[3] = coef[3];
			printf("i- coef :%d- %f, %f, %f, %f - lb:%d, sameLb:%d\n", i, coefs[i][0], coefs[i][1], coefs[i][2], coefs[i][3], lb, lb2);

			printf("before ds.m_points[i].vec_pt_3d_db size : %d\n", ds.m_points[i].vec_H_voxels[j].vec_pt_3d_db.size());

			// 벡터 복사
			ds.m_points[i].vec_H_voxels[j].vec_pt_3d_db.clear();
			ds.m_points[i].vec_H_voxels[j].vec_pt_3d_db.resize(tmp_inlierData.size());
			copy(tmp_inlierData.begin(), tmp_inlierData.end(), ds.m_points[i].vec_H_voxels[j].vec_pt_3d_db.begin());

			printf("after ds.m_points[i].vec_pt_3d_db size : %d\n\n", ds.m_points[i].vec_H_voxels[j].vec_pt_3d_db.size());
		}
	}
	printf("level1 - avgFit_distErr:%f\n", sumFit_distErr / nLabel);
	ds.H_planeFitResults(ds.m_points, ds.m_points.size(), true, color);//Display
	...
	for (int i = 0; i < nLabel; ++i)
	{
		for (int j = 0; j < ds.m_points[i].vec_H_voxels.size(); ++j)
		{
			vector<Point_3D_DB> &vec_label_points = ds.m_points[i].vec_H_voxels[j].vec_pt_3d_db;
	...
	for (int i = 0; i < ncam; ++i)
	{
		...
		*/
		for (int n = 0; n < nLabel; ++n)
		{
			...
			float **lv1_coefs = new float *[ss];
			for (int n = 0; n < ss; ++n)
			{
				lv1_coefs[n] = new float[4];
			}

			for (int k = 0; k < ds.m_points[n].vec_H_voxels.size(); ++k)  // ss크기.. -> 첫번째 레벨의 복셀 크기..(포인트가 있는..)
			{
				lv1_coefs[k][0] = ds.m_points[n].vec_H_voxels[k].coef[0];
				lv1_coefs[k][1] = ds.m_points[n].vec_H_voxels[k].coef[1];
				lv1_coefs[k][2] = ds.m_points[n].vec_H_voxels[k].coef[2];
				lv1_coefs[k][3] = ds.m_points[n].vec_H_voxels[k].coef[3];

				vector<Point_3D_DB> &vec_label_points = ds.m_points[n].vec_H_voxels[k].vec_pt_3d_db;
			...
			if (Voxel_Labeling_Method == 1)
			{
				...
				for (int j = 0; j < eachLabelPt.size(); ++j)
				{
					...
					//if(n==29)
					//	printf("j:%d, eachLabelPt[j][0].clusterID: %d\n", j, val);
					...
					for (int yy = 0; yy < label_img.rows; ++yy)
					{
						for (int xx = 0; xx < label_img.cols; ++xx)
						{
							...
							ushort usLabel = tmpLabel_img.at<ushort>(Point(xx, yy));

							if (usLabel == 0) continue;

							int feature_x = xx;
							int feature_y = yy;

							DB_Point tmp;
							tmp.clusterID = (int)usLabel;   //배경이 0이라서... id는 1부터 시작함.
							tmp.x_2d = feature_x;
							tmp.y_2d = feature_y;
							...
							float n[3] = { lv1_coefs[tmp.clusterID - 1][0], lv1_coefs[tmp.clusterID - 1][1], lv1_coefs[tmp.clusterID - 1][2] };
							...
							float tt = (n[0] * pt1[0] + n[1] * pt1[1] + n[2] * pt1[2] + lv1_coefs[tmp.clusterID - 1][3])
							...
							vec_tmpLabel_2d_3d_pts[feature_y*down_img.cols + feature_x].push_back(tmp); // 2d 포인트에서 3d 공간으로 역사영한 평면의 교점.. 저장.
				...
				for (int j = 0; j < vec_tmpLabel_2d_3d_pts.size(); ++j)
				{
					...
					//if (n == 29)
					//{
					//	printf(" vec_tmpLabel_2d_3d_pts[j].size(): %d\n", vec_tmpLabel_2d_3d_pts[j].size());
					//	printf("vec_tmpLabel_2d_3d_pts[j] clusterID: %d\n", vec_tmpLabel_2d_3d_pts[j][0].clusterID);
					//}
				...
				for (int lb = 0; lb < vec_each_lb_pts.size(); ++lb)
				{
					...
					for (int k = 0; k < vec_each_lb_pts[lb].size(); ++k)
					{
						...
						//ushort val = voxel_label_img.at<ushort>(Point(feature_x, feature_y));
						//if (val == 0) continue;
						...
						float mag = sqrt((vv[0] * vv[0]) + (vv[1] * vv[1]) + (vv[2] * vv[2]));
						float v[3] = { vv[0], vv[1], vv[2] };  //직선의 방정식의 방향 단위 벡터
						float n[3] = { lv1_coefs[val - 1][0], lv1_coefs[val - 1][1], lv1_coefs[val - 1][2] };
						float v_dot_n = v[0] * n[0] + v[1] * n[1] + v[2] * n[2];
						if (v_dot_n == 0)
						{
							printf("내적이 0 이므로, 직선과 평면이 나란한 경우다!\n");
						...
						float tt = (n[0] * pt1[0] + n[1] * pt1[1] + n[2] * pt1[2] + lv1_coefs[val - 1][3])
							/ (n[0] * (pt1[0] - pt2[0]) + n[1] * (pt1[1] - pt2[1]) + n[2] * (pt1[2] - pt2[2]));
						...
						voxel_3dPt_img.at<Vec3f>(feature_y, feature_x)[0] = Pi[0];         //vec3d는 double형 벡터
						voxel_3dPt_img.at<Vec3f>(feature_y, feature_x)[1] = Pi[1];
						voxel_3dPt_img.at<Vec3f>(feature_y, feature_x)[2] = Pi[2];

						float cam_plane_dist = sqrt((pt1[0] - Pi[0])*(pt1[0] - Pi[0]) + (pt1[1] - Pi[1])*(pt1[1] - Pi[1]) + (pt1[2] - Pi[2])*(pt1[2] - Pi[2]));
						voxel_3dPt_dist_img.at<float>(feature_y, feature_x) = cam_plane_dist;
			...
			for (int j = 0; j < eachLabelPt.size(); ++j)
			{
				if (eachLabelPt[j].size() < nThresh_points)
					continue; 
		...
		Point minLoc;
		Point maxLoc;
		cv::minMaxLoc(voxel_3dPt_dist_img, &minVal, &maxVal, &minLoc, &maxLoc);
		Mat voxel_3dPt_dist_32ibt_img = Mat::zeros(down_img.rows, down_img.cols, CV_32FC1);
		// 50~ 150 사이로 정규화 시켜보장
		for (int y = 0; y < voxel_3dPt_dist_32ibt_img.rows; ++y)
		{
			for (int x = 0; x < voxel_3dPt_dist_32ibt_img.cols; ++x)
			{
				float val = (voxel_3dPt_dist_img.at<float>(y, x)-minVal)/(maxVal-minVal) * (1000-50) + 50;
				voxel_3dPt_dist_32ibt_img.at<float>(y,x) = val;
			}
		}
		...
		if (0)
		{
			...
			// level 0 시작...
			vector<vector<DB_Point>> eachLabelPt;
			int ss = ds.m_points.size();
			eachLabelPt.resize(ss);

			// level 0단계...
			for (int k = 0; k < ds.m_points.size(); ++k)
			{
				if (ds.m_points[k].vec_pt_3d_db.size() < nThresh_points)  // 복셀안의 포인트 수가 4개 미만이면, 진행 X
				{
					continue;
				}
				for (int l = 0; l < ds.m_points[k].vec_pt_3d_db.size(); ++l)
				{
					int imgs_size = ds.m_points[k].vec_pt_3d_db[l].img_IDs.size();
					for (int m = 0; m < imgs_size; ++m)
					{
						if (ds.m_points[k].vec_pt_3d_db[l].img_IDs[m] == i)
						{
							float feature_x = ds.m_points[k].vec_pt_3d_db[l].vec_2d_pt[m].x;
							float feature_y = ds.m_points[k].vec_pt_3d_db[l].vec_2d_pt[m].y;
							...
							DB_Point tmp_db;
							int color_lb = ds.m_points[k].vec_pt_3d_db[l].clusterID - 1;
							tmp_db.clusterID = color_lb + 1;  //배경을 0 라벨로 주기위해... 
							tmp_db.x = ds.m_points[k].vec_pt_3d_db[l].x;
							tmp_db.y = ds.m_points[k].vec_pt_3d_db[l].y;
							tmp_db.z = ds.m_points[k].vec_pt_3d_db[l].z;
							tmp_db.x_2d = feature_x;
							tmp_db.y_2d = feature_y;
							tmp_db.pt_3d_id = ds.m_points[k].vec_pt_3d_db[l].pt_3d_id;
							tmp_db.pt_2d_id = ds.m_points[k].vec_pt_3d_db[l].pt_2d_ids[m];
