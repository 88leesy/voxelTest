...
#define EPSILON (0.08)  // distance for clustering, metre^2
	...
	//float reSize_H = 852, reSize_W = 480;
	...
	ds.voxelFitting4(ds.m_points, ds.getTotalPointSize(), color, nLabel);
	...
	*/
	...
	{
		if (nThresh_points > vec_label_points[i].size())
		{
	...
	int avgEachVoxelPtCnt = sumEachVoxelPtCnt / nEachVoxel_lbCnt;
	...
	*/
	string save_path = "F:/_voxelFeatureMap9_single/" + dataname + "_3d_nearest_centroid.txt";
	...
	std::ofstream oStream_dbData("F:/_voxelFeatureMap9_single/" + dataname + "db_data.txt", ios::out | ios::binary); //ios::app => 이어쓰기
	...
	for (int i = 0; i < ncam; ++i)
	{
		...
		int c_w = img.cols / 2;
		int c_h = img.rows / 2;
		//Mat label_img = Mat::zeros(img.rows, img.cols, CV_16U);
		...
		*/
		...
		//Mat kp_label_img = Mat::zeros(img.rows, img.cols, CV_8U);
		...
		*/
		//Mat gt_kp_label_img = Mat::zeros(img.rows, img.cols, CV_8U);
		...
		string save_path_rgb = "F:/_voxelFeatureMap9_single/" + dataname + "/" + img_path_split[0] + ".png";
		string save_path = "F:/_voxelFeatureMap9_single/" + dataname + "/" + img_path_split[0] + "_label_" + ".png";
		string save_path2 = "F:/_voxelFeatureMap9_single/" + dataname + "/" + img_path_split[0] + "_kp_" + ".png";

		//string save_path = "F:/_voxelFeatureMap9_single/" + dataname + names[i];
		//string save_path2 = "F:/_voxelFeatureMap9_single/" + dataname + names[i];
