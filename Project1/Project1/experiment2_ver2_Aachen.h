...
#define EPSILON (0.08)  // distance for clustering, metre^2
	...
	*/
	string save_path = "F:/_voxelFeatureMap_Aachen/_pair_Voxel_set/_3d_nearest_centroid.txt";
	...
	*/
	...
	//이미지별로 불러오기
	vector<DB_Image_Aachen> vec_db_image;
	for (int i = 0; i < ncam; ++i)
	{
		...
		*/
			...
			for (int l = 0; l < vec_label_points[k].size(); ++l)
			{
				...
				for (int m = 0; m < imgs_size; ++m)
				{
					if (vec_label_points[k][l].img_IDs[m] == i)
					{
						...
						feature_x = int(feature_x / ((float)img.cols / reSize_W));
						feature_y = int(feature_y / ((float)img.rows / reSize_H));
		...
		tmp_db_img.Cx = c_w;
		tmp_db_img.Cy = c_h;
		tmp_db_img.r = camera_data[i].radial;
	...
	std::ofstream oStream_first("F:/_voxelFeatureMap_Aachen/_pair_Voxel_set/db_data_first.txt", ios::out | ios::binary); //ios::app => 이어쓰기
	std::ofstream oStream_second("F:/_voxelFeatureMap_Aachen/_pair_Voxel_set/db_data_second.txt", ios::out | ios::binary); //ios::app => 이어쓰기
	...
	for (int i = 0; i < ncam - 1; ++i)
	{
		...
		for (int tar_im_id = i + 1; tar_im_id < ncam; ++tar_im_id)
		{
			...
			string img_path2 = "F:/Aachen-Day-Night dataset/images/images_upright/" + names[tar_im_id];
			...
			//사이즈가 동일한 것에 대해서만 piar를 만들자.
			if (img.cols != img2.cols || img.rows != img2.rows) continue;
			...
			if (same_pt_cnt >= min_pt_cnt_thresh && same_pt_cnt >= 70)  //특정 개수(최소 70개는 있자) 이상일 때에만 이미지 페어 저장, DB 저장.
			{
				...
				float target_size = 256;
				float scale = max(img2.cols, img2.rows);
				scale = target_size / scale;
				int reSize_W = img2.cols*scale;
				int reSize_H = img2.rows*scale;
				...
				for (int j = 0; j < ref_label_tmp_dbPt.size(); ++j)
				{
					...
					//poly line 확인해보는... debug
					Scalar cvColor(color[j][0], color[j][1], color[j][2]);
					polylines(down_img, &pts, &npts, 1, true, cvColor, 1);
				...
				for (int j = 0; j < tar_label_tmp_dbPt.size(); ++j)
				{
					...
					//poly line 확인해보는... debug
					Scalar cvColor(color[j][0], color[j][1], color[j][2]);
					polylines(down_img2, &pts, &npts, 1, true, cvColor, 1);
				}
				...
				string ref_file_name = to_string(i) + '_' + to_string(tar_im_id) + '_' + "ref_" + line_str[0] + '_' + line_str[1];
				...
				string tar_file_name = to_string(i) + '_' + to_string(tar_im_id) + '_' + "tar_" + line_str[0] + '_' + line_str[1];
				tar_file_name.replace(tar_file_name.find("jpg"), 3, "png");

				string save_path = "F:/_voxelFeatureMap_Aachen/_pair_Voxel_set/" + ref_file_name;
				imwrite(save_path, down_img);
				save_path.replace(save_path.find("db"), 2, "label_db");
				imwrite(save_path, ref_label_img);
				save_path.replace(save_path.find("label"), 5, "kp");
				imwrite(save_path, kp_label_img);

				string save_path2 = "F:/_voxelFeatureMap_Aachen/_pair_Voxel_set/" + tar_file_name;
				imwrite(save_path2, down_img2);
				save_path2.replace(save_path2.find("db"), 2, "label_db");
				imwrite(save_path2, tar_label_img);
				save_path2.replace(save_path2.find("label"), 5, "kp");
				imwrite(save_path2, kp_label_img2);


				pair<DB_Image_Aachen, DB_Image_Aachen> pair_DB_img;
				DB_Image_Aachen ref_tmp_dbImg, tar_tmp_dbImg;
				...
				oStream_first << pair_DB_img.first.img_ID << " " << pair_DB_img.first.img_path << " " << pair_DB_img.first.quat[0] << " " << pair_DB_img.first.quat[1]
					...
					<< " " << pair_DB_img.first.camCent[2] << " " << pair_DB_img.first.focal_len 
					<< " " << pair_DB_img.first.Cx << " " << pair_DB_img.first.Cy << " " << pair_DB_img.first.r << " ";

				//printf("무엇이 문제인고 1111 \n");
				...
				oStream_second << pair_DB_img.second.img_ID << " " << pair_DB_img.second.img_path << " " << pair_DB_img.second.quat[0] << " " << pair_DB_img.second.quat[1]
					...
					<< " " << pair_DB_img.second.camCent[2] << " " << pair_DB_img.second.focal_len 
					<< " " << pair_DB_img.second.Cx << " " << pair_DB_img.second.Cy << " " << pair_DB_img.second.r << " ";;
