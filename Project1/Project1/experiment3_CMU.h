	...
	*/
	...
	for (int i = 0; i < ncam; ++i)
	{
		...
		*/
			...
			for (int l = 0; l < vec_label_points[k].size(); ++l)
			{
				...
				for (int m = 0; m < imgs_size; ++m)
				{
					if (vec_label_points[k][l].img_IDs[m] == i)
					{
						...
						//float x = camera_data[i].m[0][0] * vec_label_points[k][l].x
						//		+ camera_data[i].m[0][1] * vec_label_points[k][l].y
						//		+ camera_data[i].m[0][2] * vec_label_points[k][l].z;

						//float y = camera_data[i].m[1][0] * vec_label_points[k][l].x
						//		+ camera_data[i].m[1][1] * vec_label_points[k][l].y
						//		+ camera_data[i].m[1][2] * vec_label_points[k][l].z;

						//float z = camera_data[i].m[2][0] * vec_label_points[k][l].x
						//		+ camera_data[i].m[2][1] * vec_label_points[k][l].y
						//		+ camera_data[i].m[2][2] * vec_label_points[k][l].z;
						...
						//x = fx * x;
						//y = fy * y;
						//float test_feature_x = x / z;
						//float test_feature_y = y / z;

						//float pt[3] = { vec_label_points[k][l].x , vec_label_points[k][l].y, vec_label_points[k][l].z };
						//float pt_2d[2] = { 0 };
						//camera_data[i].GetProjectionPoint(pt, pt_2d);

						//float test_x = vec_label_points[k][l].vec_2d_pt[m].x ;
						//float test_y = vec_label_points[k][l].vec_2d_pt[m].y ;
		...
		string save_path_rgb = "F:/_voxelFeatureMap_CMU/_single_Voxel_set/" + dataname + names[i];
		string save_path = "F:/_voxelFeatureMap_CMU/_single_Voxel_set/" + dataname + line_str[0] + "/label_" + line_str[1];
		string save_path2 = "F:/_voxelFeatureMap_CMU/_single_Voxel_set/" + dataname + line_str[0] + "/kp_" + line_str[1];
