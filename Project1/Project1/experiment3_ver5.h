	...
	vector<string>& names, vector<int>& ptc, int cubeSize)
{
	...
	if (1)
	{
		...
		vector<double> vec_gt_mean;

		for (int idx = 0; idx < ncam; ++idx)
		{
			string img_path = "F:/RPNet_test/data/absolute_cambridge/" + dataname + names[idx];
			cout << names[idx] << endl;
			...
			//int size = vec_db_pt_3d.size();
			//for (int i = 0; i < size; ++i)
			//{
			//	int imgSize = vec_db_pt_3d[i].img_IDs.size();

			//	for (int m = 0; m < imgSize; ++m)
			//	{
			//		if (vec_db_pt_3d[i].img_IDs[m] == idx)  //vec_db_pt_3d[0].vec_pt_3d_db[i].img_IDs[m], vec_label_points[l].img_IDs[m]
			//		{
			//			//float feature_x = vec_db_pt_3d[i].vec_2d_pt[m].x - c_w;
			//			//float feature_y = vec_db_pt_3d[i].vec_2d_pt[m].y - c_h;

			//			//float reSize_H = 256, reSize_W = 456;
			//			//feature_x = feature_x / (img.cols / reSize_W);
			//			//feature_y = feature_y / (img.rows / reSize_H);

			//			float feature_x = vec_db_pt_3d[i].vec_2d_pt[m].x;
			//			float feature_y = vec_db_pt_3d[i].vec_2d_pt[m].y;
			...
			//			DB_Point tmp_db;
			//			tmp_db.x = vec_db_pt_3d[i].x;
			//			tmp_db.y = vec_db_pt_3d[i].y;
			//			tmp_db.z = vec_db_pt_3d[i].z;
			//			tmp_db.x_2d = feature_x;
			//			tmp_db.y_2d = feature_y;
			//			tmp_db.pt_3d_id = vec_db_pt_3d[i].pt_3d_id;
			//			tmp_db.pt_2d_id = vec_db_pt_3d[i].pt_2d_ids[m];

			//			eachLabelPt.push_back(tmp_db);
			//			tmp_features_2d.push_back(Point2d(feature_x, feature_y));   // donw_c_w, yy - donw_c_h
			//			tmp_features_3d.push_back(Point3d(tmp_db.x, tmp_db.y, tmp_db.z));
			//		}
			//	}
			//}

			tmp_features_2d = features_2d[idx];
			tmp_features_3d = features_3d[idx];

			double scale = img.rows / 256.0;
			...
			A_matrix.at<double>(1, 2) = params[3];
			A_matrix.at<double>(2, 2) = 1;
			bool correspondence = EstimatePoseByPnP(tmp_features_2d, tmp_features_3d,
				camera_data[idx].f, A_matrix, R_matrix, t_matrix);
			...
			float q2[4];
			camera_data[idx].GetQuaternionRotation(q2);

			float dot_prod = gt_q[0] * q2[0] + gt_q[1] * q2[1] + gt_q[2] * q2[2] + gt_q[3] * q2[3];
				...
				//for (int ind = 0; ind < tmp_features_3d.size(); ++ind)
			{
				...
				int xxx = Point2f.x + c_w + 0.5;
				int yyy = Point2f.y + c_h + 0.5;
				circle(img, Point(xxx, yyy), 3, Scalar(255, 0, 0), -1);
				...
				int xx = Point2f_est.x + c_w + 0.5;
				int yy = Point2f_est.y + c_h + 0.5;
				circle(img, Point(xx, yy), 3, Scalar(0, 255, 0), -1);


				//GT
				int feature_x = features_2d[idx][ind].x + c_w;
				int feature_y = features_2d[idx][ind].y + c_h;
				//int feature_x = tmp_features_2d[ind].x;
				//int feature_y = tmp_features_2d[ind].y;
				circle(img, Point(feature_x, feature_y), 3, Scalar(0, 0, 255), -1);
		...
		auto n = vec_gt_mean.size();
		printf("vec_gt_mean:%f, \n", accumulate(vec_gt_mean.begin(), vec_gt_mean.end(), 0.0) / n);
...
#define EPSILON (0.08)  // distance for clustering, metre^2
	...
	*/
	...
	ds.printResults(vec_inlierData, vec_inlierData.size(), true, color);  //plane fitting view
	...
	if (Voxel_Labeling_Method == 1)
		...
		absPath = "F:/_voxelFeatureMap12_single/";
	else
		absPath = "F:/_voxelFeatureMap10_single/";

	/*
	Write 3d centroid points
	*/
	string save_path = absPath + dataname + "_3d_nearest_centroid.txt";
	...
	std::ofstream oStream_dbData(absPath + dataname + "db_data.txt", ios::out | ios::binary); //ios::app => 이어쓰기
	...
	for (int i = 0; i < ncam; ++i)
	{
		...
		*/
		...
		DB_img.img_ID = i;
		DB_img.distort = camera_data[i].radial;
		...
		for (int yy = 0; yy < label_img.rows; ++yy)
		{
			for (int xx = 0; xx < label_img.cols; ++xx)
			{
				ushort val = lb_data_ptr[yy*label_img.cols + xx];
				if (val == 0) continue;
				...
				vec_features_2d.push_back(Point2d(xx - donw_c_w, yy - donw_c_h));
		...
		bool correspondence = EstimatePoseByPnP(vec_features_2d, vec_features_3d, focal_len, A_matrix, R_matrix, t_matrix);
		...
		*/
		...
		string save_path_rgb = absPath + dataname + img_path_split[0] + ".png";
		string save_path = absPath + dataname + img_path_split[0] + "_label_" + ".png";
		string save_path2 = absPath + dataname + img_path_split[0] + "_kp_" + ".png";
		string save_path3 = absPath + dataname + img_path_split[0] + "_3dpts" + ".yml";
		string save_path4 = absPath + dataname + img_path_split[0] + "_contour" + ".png";
		string save_path5 = absPath + dataname + img_path_split[0] + "_fillcontour" + ".png";
		cout << save_path << endl;
		...
		//printf("66666666666666666666666666666666666666666666666666666666666666666666\n");
