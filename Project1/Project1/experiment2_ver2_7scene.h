...
#define EPSILON (0.08)  // distance for clustering, metre^2
	...
	*/
	string save_path = "F:/_voxelFeatureMap_RGBD_Dataset7_Scenes/_pair_Voxel_set/" + dataname + "_3d_nearest_centroid.txt";
	...
	*/
	...
	for (int i = 0; i < ncam; ++i)
	{
		...
		*/
		...
		tmp_db_img.focal_len = fx / scaleX;
		tmp_db_img.focal_len = fy / scaleY;
	...
	std::ofstream oStream_first("F:/_voxelFeatureMap_RGBD_Dataset7_Scenes/_pair_Voxel_set/" + dataname + "db_data_first.txt", ios::out | ios::binary); //ios::app => 이어쓰기
	std::ofstream oStream_second("F:/_voxelFeatureMap_RGBD_Dataset7_Scenes/_pair_Voxel_set/" + dataname + "db_data_second.txt", ios::out | ios::binary); //ios::app => 이어쓰기
	...
	for (int i = 0; i < ncam - 1; ++i)
	{
		...
		for (int tar_im_id = i + 1; tar_im_id < ncam; ++tar_im_id)
		{
			...
			//string img_path2 = "F:/_RGBD_Dataset7_Scenes/" + dataname + "jpgFiles/" + names[tar_im_id];
			string img_path2 = "F:/_RGBD_Dataset7_Scenes/" + dataname + names[tar_im_id];
			...
			if (same_pt_cnt >= min_pt_cnt_thresh && same_pt_cnt >= 80)  //특정 개수(최소 70개는 있자) 이상일 때에만 이미지 페어 저장, DB 저장.
			{
				...
				vector<string> line_str = split(names[i], '.');
				vector<string> img_path_split = split(line_str[0], '_');
				string ref_file_name = img_path_split[0] + "/" + to_string(i) + '_' + to_string(tar_im_id) + '_' + "ref_" + img_path_split[1] + ".png";
				
				line_str = split(names[tar_im_id], '.');
				img_path_split = split(line_str[0], '_');
				string tar_file_name = img_path_split[0] + "/" + to_string(i) + '_' + to_string(tar_im_id) + '_' + "tar_" + img_path_split[1] + ".png";


				string save_img_path = "F:/_voxelFeatureMap_RGBD_Dataset7_Scenes/_pair_Voxel_set/" + dataname + ref_file_name;
				...
				string save_img_path2 = "F:/_voxelFeatureMap_RGBD_Dataset7_Scenes/_pair_Voxel_set/" + dataname + tar_file_name;
