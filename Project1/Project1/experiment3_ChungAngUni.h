	...
	vector<string>& names, vector<int>& ptc, int cubeSize)
{
	...
	//따라서 get처럼 스트림에서 문자를 빼내는 것이 아니라, 그 문자는 그냥 스트림에 남아있게 된다.
	if (in.peek() == 'N')   
	{
	...
	for (int i = 0; i < npoint; ++i)
	{
		...
		for (int j = 0; j < npj; ++j)
		{
			...
			//float pnp_feature_x = f_fx * x / z ;
			//float pnp_feature_y = f_fy * y / z ;
...
#define EPSILON (0.08)  // distance for clustering, metre^2
	...
	*/
	string save_3dpt_path = "F:/_voxelFeatureMap_ChungAngUni_Scenes/single_Voxel_set/" + dataname + "_3d_nearest_centroid.txt";
	...
	std::ofstream oStream_dbData("F:/_voxelFeatureMap_ChungAngUni_Scenes/single_Voxel_set/" + dataname + "db_data.txt", ios::out | ios::binary); //ios::app => 이어쓰기
	...
	for (int i = 0; i < ncam; ++i)
	{
		...
		//ArtCenter는 고정 focal length와 pricipal points를 주지 않음.
		if (dataname != "ArtCenter/")
		{
			fx = f_fx;
			fy = f_fy;
		...
		*/
		...
		reName.replace(reName.find("_"), 1, "/");

		vector<string> path_split = split(reName, '_');
		string ref_file_name = path_split[0] + path_split[1];

		DB_img.img_path = ref_file_name;
		...
		//string save_path_rgb = "F:/_voxelFeatureMap_RGBD_Dataset7_Scenes/single_Voxel_set/" + dataname + line_str[0] + ".jpg";
		//string save_path = "F:/_voxelFeatureMap_RGBD_Dataset7_Scenes/single_Voxel_set/" + dataname + "label_" + line_str[0] + ".jpg";
		//string save_path2 = "F:/_voxelFeatureMap_RGBD_Dataset7_Scenes/single_Voxel_set/" + dataname + "kp_" + line_str[0] + ".jpg";
		...
		string save_path_rgb = "F:/_voxelFeatureMap_ChungAngUni_Scenes/single_Voxel_set/" 
			+ dataname + img_path_split[0] + "/" + img_path_split[1] + img_path_split[2] + ".png";
		string save_path = "F:/_voxelFeatureMap_ChungAngUni_Scenes/single_Voxel_set/" 
			+ dataname + img_path_split[0] + "/" + "label_" + img_path_split[1] + img_path_split[2] + ".png";
		string save_path2 = "F:/_voxelFeatureMap_ChungAngUni_Scenes/single_Voxel_set/" 
			+ dataname + img_path_split[0] + "/" + "kp_" + img_path_split[1] + img_path_split[2] + ".png";
		...
		imwrite(save_path_rgb, down_img);
