	...
	*/
	...
	for (int i = 0; i < ncam; ++i)
	{
		...
		*/
		...
		//{
		//	printf("\n j:%d, coef : %f, %f, %f, %f\n", j, coefs[j][0], coefs[j][1], coefs[j][2], coefs[j][3]);
		//}
		...
		{	
			...
			printf("\nlabel:%d\n", j);
			printf("coef : %f, %f, %f, %f\n", coefs[j][0], coefs[j][1], coefs[j][2], coefs[j][3]);
			...
			{
				float camCent[3];
				camera_data[i].GetCameraCenter(camCent);


				//내적 계산
				//float inner = vec_voxels[j].planes[k].surfaceNomalX*camera_data[i].t[0]
				//	+ vec_voxels[j].planes[k].surfaceNomalY*camera_data[i].t[1]
				//	+ vec_voxels[j].planes[k].surfaceNomalZ*camera_data[i].t[2];

				//float absPL = sqrt( (vec_voxels[j].planes[k].surfaceNomalX*vec_voxels[j].planes[k].surfaceNomalX)
				//	+ (vec_voxels[j].planes[k].surfaceNomalY*vec_voxels[j].planes[k].surfaceNomalY)
				//	+ (vec_voxels[j].planes[k].surfaceNomalZ*vec_voxels[j].planes[k].surfaceNomalZ) );
				//float absCAM = sqrt( (camera_data[i].t[0] * camera_data[i].t[0])
				//	+ (camera_data[i].t[1] * camera_data[i].t[1])
				//	+ (camera_data[i].t[2] * camera_data[i].t[2]) );

				float inner = vec_voxels[j].planes[k].surfaceNomalX*coefs[j][0]
					+ vec_voxels[j].planes[k].surfaceNomalY*coefs[j][1]
					+ vec_voxels[j].planes[k].surfaceNomalZ*coefs[j][2];

				float absPL = sqrt((vec_voxels[j].planes[k].surfaceNomalX*vec_voxels[j].planes[k].surfaceNomalX)
					+ (vec_voxels[j].planes[k].surfaceNomalY*vec_voxels[j].planes[k].surfaceNomalY)
					+ (vec_voxels[j].planes[k].surfaceNomalZ*vec_voxels[j].planes[k].surfaceNomalZ));
				float absCAM = sqrt((coefs[j][0] * coefs[j][0])
					+ (coefs[j][1] * coefs[j][1])
					+ (coefs[j][2] * coefs[j][2]));

				float theta = acos(inner / (absPL * absCAM)) * 180/M_PI;
				if (bestT > theta)
				{
					bestT = theta;
					bestK = k;
				...
				printf("k:%d, bestK:%d, theta:%f, bestT:%f\n", k, bestK, theta, bestT);


				//float camCent[3];
				//camera_data[i].GetCameraCenter(camCent);
				//float dist = ds.calcDistance(vec_voxels[j].planes[k], camCent[0], camCent[1], camCent[2]);
				//if (distantD < dist)
				//{
				//	distantD = dist;
				//	bestK = k;
			...
			for (int l = 0; l < 4; ++l)
			{
				...
				float x = camera_data[i].m[0][0] * vec_voxels[j].planes[bestK].x[l]
						+ camera_data[i].m[0][1] * vec_voxels[j].planes[bestK].y[l]
						+ camera_data[i].m[0][2] * vec_voxels[j].planes[bestK].z[l];
																		
				float y = camera_data[i].m[1][0] * vec_voxels[j].planes[bestK].x[l]
						+ camera_data[i].m[1][1] * vec_voxels[j].planes[bestK].y[l]
						+ camera_data[i].m[1][2] * vec_voxels[j].planes[bestK].z[l];
																		
				float z = camera_data[i].m[2][0] * vec_voxels[j].planes[bestK].x[l]
						+ camera_data[i].m[2][1] * vec_voxels[j].planes[bestK].y[l]
						+ camera_data[i].m[2][2] * vec_voxels[j].planes[bestK].z[l];
				...
				int feature_x = pnp_feature_x + (reSize_W/2) + 0.5;
				int feature_y = pnp_feature_y + (reSize_H/2) + 0.5;

				circle(voxel_down_img, Point(feature_x, feature_y), 1, cvColor, -1);
				resized_contour.push_back(Point(feature_x, feature_y));

				voxel_kp_label_img.data[feature_y*voxel_kp_label_img.cols + feature_x] = 255;
			}	

			//for (int k = 0; k < eachLabelPt[j].size(); ++k)
			//{
			//	float x = camera_data[i].m[0][0] * eachLabelPt[j][k].x
			//		+ camera_data[i].m[0][1] * eachLabelPt[j][k].y
			//		+ camera_data[i].m[0][2] * eachLabelPt[j][k].z;

			//	float y = camera_data[i].m[1][0] * eachLabelPt[j][k].x
			//		+ camera_data[i].m[1][1] * eachLabelPt[j][k].y
			//		+ camera_data[i].m[1][2] * eachLabelPt[j][k].z;

			//	float z = camera_data[i].m[2][0] * eachLabelPt[j][k].x
			//		+ camera_data[i].m[2][1] * eachLabelPt[j][k].y
			//		+ camera_data[i].m[2][2] * eachLabelPt[j][k].z;
